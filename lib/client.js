window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Pinned-section + divider styles injected under this package's own tag. */
		const dswpCss = ".dswp-pinned-section{display:flex;flex-direction:column}.dswp-pinned-section>.YDXeBa_projectRow{color:var(--dsw-alias-label-primary)}.dswp-pinned-section>.dswp-pin-heading{color:var(--dsw-alias-label-tertiary);font-size:11px;line-height:16px;padding:4px 8px 0;user-select:none}.dswp-pinned-divider{height:1px;margin:4px 8px 6px;background:var(--dsw-alias-interactive-bg-hover)}";
		const dswpTagId = "dsh-workspace-pin/pinned.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(dswpTagId) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-workspace-pin";
			tag.dataset.pluginCss = dswpTagId;
			tag.textContent = dswpCss;
		...
		*/
		...
		/** localStorage key of the official workspace view store (v5), migrated on first run. */
		const LEGACY_VIEW_KEY = "dsh.workspace.view.v5";
		/** Try to inherit the official view store's preferences once, then switch to our own key. */
		function migrateLegacyView() {
			try {
				if (typeof localStorage === "undefined") return null;
				const raw = localStorage.getItem(LEGACY_VIEW_KEY);
		...
		/**
		* Create the workspace browser viewing store handle (pinned-aware fork).
		* Adds `pinnedWorkspaceIds` — the ordered list of manually pinned
		* Workspaces rendered above every sorting rule inside the pinned section —
		* and persists under its own key after one legacy-v5 migration.
		...
		*/
		function createWorkspaceViewStore() {
			...
			return (0, _deepseek_ai_dsh_client_runtime_client.defineStore)({
				init: () => ({
					groupBy: legacy?.groupBy ?? "workspace",
					orderBy: legacy?.orderBy ?? "updated",
					groupExpansion: legacy?.groupExpansion ?? {},
					sessionOrderByAccount: legacy?.sessionOrderByAccount ?? {},
					sessionUpdatedAtByAccount: legacy?.sessionUpdatedAtByAccount ?? {},
					pinnedWorkspaceIds: []
				}),
				persist: "dsh.workspace.pin.view.v1",
				actions: {
					setGroupBy: (d, mode) => {
						d.groupBy = mode;
					},
					...
					togglePin: (d, workspaceId) => {
						const ids = d.pinnedWorkspaceIds;
						if (ids.includes(workspaceId)) d.pinnedWorkspaceIds = ids.filter((id) => id !== workspaceId);
						else d.pinnedWorkspaceIds = [...ids, workspaceId];
					},
					setPinnedOrder: (d, workspaceIds) => {
						d.pinnedWorkspaceIds = [...workspaceIds];
					},
		...
		*/
		...
		//#endregion
		//#region upstream-css:packages/client/ui-workspace/src/client/rows/Rows.module.css.mjs
		...
		*/
		function ProjectRowItem({ group, onToggle, onCreate, actions, drag, pinned = false, t }) {
			const row = group;
			const label = row.workspaceId === void 0 ? t("group.ungrouped") : row.label;
			const active = group.expanded && group.containsCurrent;
			const [menuOpen, setMenuOpen] = (0, react.useState)(false);
			const canPin = row.workspaceId !== void 0 && actions !== void 0 && typeof actions.togglePin === "function";
					...
					(0, react_jsx_runtime.jsxs)("span", {
						...
						children: [actions !== void 0 && (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
							...
							onSelect: (id) => {
								setMenuOpen(false);
								if (id === "pin") {
									if (row.workspaceId !== void 0 && actions !== void 0) actions.togglePin(row.workspaceId);
									return;
								}
		...
		*/
		...
		//#endregion
		//#region upstream-css:packages/client/ui-workspace/src/client/WorkspacePicker.module.css.mjs
		...
		*/
		...
		//#endregion
		//#region upstream-css:packages/client/ui-workspace/src/client/WorkspaceBrowser.module.css.mjs
		...
		*/
		...
		function ViewOptionsMenu({ groupBy, orderBy, onGroupPick, onOrderPick, t }) {
			...
			return (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Menu, {
				...
				items: [
					...
					{
						id: "session",
						label: t("orderBy.session")
					}
				],
				selectedIds: [groupBy, orderBy],
				onSelect: (id) => {
					if (id === "workspace" || id === "flat") onGroupPick(id);
					else if (id === "manual" || id === "updated" || id === "session") onOrderPick(id);
					setOpen(false);
				},
				align: "end",
				dense: true,
		...
		function SessionTree({ useSessions, startSession, open, forkSession, workspaces, archivedSessionIds, onRenameRequest, onDeleteRequest, onSessionRename, onSessionArchive, insertWorkspaceBefore, insertSessionBefore, orderBy, groupExpansion, setGroupExpanded, sessionOrderByAccount, sessionUpdatedAtByAccount, syncSessionOrderAccount, setSessionOrder, pinnedWorkspaceIds, togglePin, setPinnedOrder, t }) {
			...
			const now = Date.now();
			const pinnedSet = (0, react.useMemo)(() => new Set(pinnedWorkspaceIds ?? []), [pinnedWorkspaceIds]);
			const isPinnedId = (workspaceId) => workspaceId !== void 0 && pinnedSet.has(workspaceId);
			/* Split render groups into the pinned section (stable pinned order,
			* always first) and the ordinary section (under the active order rule).
			* The ungrouped bucket can never be pinned. */
			const splitGroups = (0, react.useMemo)(() => {
				const pinned = [];
				const ordinary = [];
				const byId = new Map();
				for (const group of groups) byId.set(group.key, group);
				for (const id of pinnedWorkspaceIds ?? []) {
					const group = byId.get(id);
					/* v8 ignore next -- stale pinned ids are pruned by retainAccountKeys; keep the guard cheap. */
					if (group === void 0) continue;
					pinned.push(group);
				}
				for (const group of groups) {
					if (group.workspaceId !== void 0 && pinnedSet.has(group.workspaceId)) continue;
					ordinary.push(group);
				}
				/* "会话优先": the workspace hosting the current session rises to the
				* head of the ordinary section (after any pinned rows). */
				if (orderBy === "session" && currentGroup !== void 0 && !pinnedSet.has(currentGroup)) {
					const at = ordinary.findIndex((group) => group.workspaceId === currentGroup);
					if (at > 0) {
						const [moved] = ordinary.splice(at, 1);
						ordinary.unshift(moved);
					}
				}
				return { pinned, ordinary };
			}, [groups, pinnedWorkspaceIds, pinnedSet, orderBy, currentGroup]);
			...
			const commitWorkspaceDrag = (activeDrag, over) => {
				...
				/* Pinned-section drags reorder ONLY the local pinned list — the
				* underlying Host workspace order stays untouched. */
				if (activeDrag.pinned) {
					const currentPinned = pinnedWorkspaceIds ?? [];
					const nextPinned = currentPinned.filter((id) => id !== activeDrag.workspaceId);
					const rowIndex = currentPinned.indexOf(over.id);
					if (rowIndex === -1) return;
					const anchor = over.half === "before" ? over.id : currentPinned[rowIndex + 1];
					if (anchor === activeDrag.workspaceId) return;
					const anchorIndex = anchor === void 0 ? nextPinned.length : nextPinned.indexOf(anchor);
					nextPinned.splice(anchorIndex === -1 ? nextPinned.length : anchorIndex, 0, activeDrag.workspaceId);
					setPinnedOrder(nextPinned.map((id) => id));
					return;
				}
				if (isPinnedId(over.id)) return;
			...
			const workspaceDropAtListStart = workspaceDrag?.over?.id === (splitGroups.ordinary[0]?.workspaceId) && workspaceDrag?.over?.half === "before";
			/* One row renderer shared by the pinned section and the ordinary
			* section: `pinned` drives the toggle label and the drag target. */
			const renderGroupRow = (group, pinned) => {
				...
				const hoverWorkspace = workspaceId === void 0 ? void 0 : (half) => {
					setWorkspaceDrag((active) => {
						...
						/* Never show a drop marker across sections: pinned drags may
						* only hover pinned rows and ordinary drags only ordinary ones. */
						if (active.pinned !== (pinned === true)) return active;
						return {
							...active,
							over: {
								id: workspaceId,
					...
					children: [
						(0, react_jsx_runtime.jsx)(ProjectRowItem, {
							...
							actions: group.workspaceId === void 0 ? void 0 : {
								...
								togglePin: () => {
									/* v8 ignore next -- narrowing guard: the actions object exists only for real-workspace groups. */
									if (group.workspaceId !== void 0) togglePin(group.workspaceId);
								}
							}
						}),
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsxs)("div", {
						...
						}), splitGroups.pinned.length > 0 && (0, react_jsx_runtime.jsxs)("div", {
							className: "dswp-pinned-section",
							role: "group",
							"aria-label": t("pinned.section"),
							children: splitGroups.pinned.map((group) => renderGroupRow(group, true))
						}), splitGroups.pinned.length > 0 && (0, react_jsx_runtime.jsx)("div", {
							className: "dswp-pinned-divider",
							role: "separator",
							"aria-hidden": "true"
						}), splitGroups.ordinary.map((group) => renderGroupRow(group, false))]
					}),
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: wide && (normalizedQuery !== "" ? (0, react_jsx_runtime.jsx)(SearchResults, {
							...
							setPinnedOrder: actions.setPinnedOrder,
							archivedSessionIds,
							startSession,
							open,
		...
		*/
		...
		/** Dictionary namespace owned by this plugin (own NS so the official
		* "workspace" namespace stays untouched — two bundles registering the same
		* NS+locale would collide). */
		const NS = "workspace-pin";
		/**
		...
		*/
		...
		/**
		* Register the browser once its slot declaration is on the ledger at a
		* LOWER priority (-1) than the official occupant (0) so this pinned-aware
		* WorkspaceBrowser shadows it. The picker slot stays owned by the official
		* bundle; we deliberately do NOT register conversation.hero.workspace.
		* Inject factories return plain callbacks; data reads use the
		...
		*/
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(NS, {
				zh,
				en
			}), "dsh-workspace-pin: dictionaries");
			...
			ctx.slots.inject("sidebar.workspaces", () => ctx.slots.register({
				...
				/* We take over the official ui-workspace entry (disabled in the
				* profile patch), so we declare its children slot ourselves and
				* priority can stay at the default 0. */
			...
			/* Restore the hero picker registration the official entry used to
			* own — without it the blank-session workspace picker disappears. */
